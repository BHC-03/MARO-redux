...
const userReducer = (state:loginresults = initialState,action:userActionType)=>{
    switch(action.type){
        ...
        case ActionTypes.FAILED_LOG_IN:
            return  {...initialState,err:action.payload.err};
        ...
        case ActionTypes.SESSION_LOG_IN:
            return {...state,email:action.payload.email,token:action.payload.token,photo_url:'',id:action.payload.id,username:action.payload.username}
        default:
            return state
    }
}
